...
- [x] held-out エピソードで multi-step 予測の誤差劣化を測定する (1, 5, 10 step, 潜在空間 MSE) — RoboMIND: h=1 全次元 MSE 0.019, h=10 0.197
- [x] コピーベースライン (前フレームをそのまま返す) と比較する — RoboMIND 信号次元: h=1 で既にコピーに負け (Pred/Copy=1.07), h=10 で 0.79
- 判断基準: ホライズンに対して指数的に劣化 → ワールドモデルとして計画に使えない → **全次元は線形だが信号次元 h=1 でコピーに負ける (berkeley では勝つ). RoboMIND の方が深刻**
- [x] 信号/ノイズ次元分離 MSE 分析 — berkeley: 信号 51 dims, 希釈 73%. RoboMIND: 信号 40 dims, 希釈 79%. RoboMIND の方がノイズ比率が高い
