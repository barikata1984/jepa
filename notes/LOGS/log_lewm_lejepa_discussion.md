...
1. **実機データでの学習効率**: 少量データで LeWM が機能するか. SIGReg の低データ多様性での劣化が焦点
2. **触覚・マルチモーダル JEPA ワールドモデル**: GelSight 等の触覚を追加モダリティとして統合
3. **サプライズ駆動の能動データ収集**: VoE フレームワークを内発的動機づけに使う
...
- → いずれも純粋な評価で終わると弱い. まず動かして"どこで壊れるか"を観察し, そこから解決すべき問題と提案手法を決める方が現実的

### 論文中の既知の穴 (実機で問題になりうる点)

LeWM §6 から:

1. **短ホライゾン制約**: H=5 ステップ (フレームスキップ込みで 25 環境ステップ). 実機タスクには不足の可能性
2. **データカバレッジへの依存**:"十分なインタラクションカバレッジ"が前提. 実機データは疎
3. **低データ多様性で SIGReg が劣化 (TwoRoom 問題)**: 固有次元が低いデータで, 高次元潜在空間の等方ガウス正則化がミスマッチを起こす. 実機の固定セットアップは視覚的多様性が低く, 同様の問題が起きうる → **最も有望な穴**
4. **行動ラベルへの依存**: (observation, action) ペアが必須

LeJEPA から:

5. **i.i.d. 仮定**: 理論の前提. 系列データへの拡張は未議論 (LeWM ではステップワイズ適用で暗黙的に回避)
...
- 固有次元は事前にわからないため, 環境ごとに潜在次元を合わせるのは LeWM のタスク非依存の設計方針に反する
...
- **低データ多様性での SIGReg 劣化の方が, 理論・実験ともに掘りがいがある**と結論
